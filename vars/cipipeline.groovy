  ...
  node() {
    ...
    if (env.gitbrname == "main") {
      ...
      stage('Docker Build & Push') {
        ...
        sh "docker build -t 739561048503.dkr.ecr.us-east-1.amazonaws.com/${env.appName}:${env.GIT_COMMIT} ."
        sh "docker push 739561048503.dkr.ecr.us-east-1.amazonaws.com/${env.appName}:${env.GIT_COMMIT}"
...
//          sh "argocd app create ${env.appName} --upsert --project default --sync-policy auto --repo https://github.com/raghudevopsb79/roboshop-helm --path chart --dest-namespace ${env.namespace} --dest-server https://kubernetes.default.svc --values ../env-dev/${env.appName}.yaml --helm-set image=739561048503.dkr.ecr.us-east-1.amazonaws.com/roboshop-cart:${env.GIT_COMMIT}"
//        }
