    ...
    if (env.gitbrname == "main") {
      ...
      stage('Deploy to Dev') {
        ...
        maskPasswords(varPasswordPairs: [[password: argocd_password, var: 'argocd_password']]) {
          ...
          sh "cd helm/chart ;argocd app create ${env.appName} --project default --sync-policy auto --repo https://github.com/raghudevopsb79/roboshop-helm --path chart --dest-namespace ${env.namespace} --dest-server https://kubernetes.default.svc"
        }
